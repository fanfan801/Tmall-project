    ...
    return {
        init: function() {
            ...
            $.ajax({
                ...
                //将li元素添加到排序前的数组中。
                $('.liebiaoye li').each(function(index, element) { //element:原生的元素对象
                ...
                // 默认排序
                $('.paixu button').eq(0).on('click', function() {
                    // alert(1);
                    //遍历渲染。
                    $.each($array_default, function(index, value) { //value就是li元素
                        $liebiaoye.append(value);
                    });
                });

                // 升序
                $('.paixu button').eq(1).on('click', function() {
                ...
                //降序
                $('.paixu button').eq(2).on('click', function() {
                    for (let i = 0; i < $array.length - 1; i++) {
                        for (let j = 0; j < $array.length - i - 1; j++) {
